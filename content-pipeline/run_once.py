...
def _split_board(board_title: str) -> tuple[str, str]:
    ...
    옛 보드(일대·요금·이용료)는 2026-07-16 신규 발행 중단됐지만 기존 글 재합성
    (backfill) 호환을 위해 매핑을 유지한다.
    """
    BOARD_MAP = {
        "스카이차":      ("스카이차", "스카이차"),
        "스카이차 일대":  ("일대",    "스카이차"),
...
# 신규 발행 대상 보드 (slug, title) — 2026-07-16 사장님 확정 (검색량 실측).
# 옛 3종(스카이차-일대·스카이차-요금·스카이차-이용료)은 DB·기존 글·네비에 그대로
# 남지만 신규 발행에서만 제외. 대신 신규 3종(임대·업체·근처)을 발행 대상에 투입.
# ("대여"는 금지어 — "임대"만 사용.)
# DB boards 테이블에는 기존 8 + 신규 3 = 11개가 존재해야 함 (add-boards SQL 선행 필수).
BOARDS = [
    ("스카이차",         "스카이차"),
    ("스카이차-임대",     "스카이차 임대"),
    ("스카이-작업차",     "스카이 작업차"),
...
# 보드 검색량 가중 (메모리 실측: 스카이차 > 상업 클러스터(임대/업체/근처/비용/가격)
# > 고소작업차 > 고소작업차량). 발행 시 이 가중으로 보드를 뽑아 검색가치 높은
# 키워드부터 채운다. 단 과도한 쏠림은 네이버 패턴화 위험이라 완만하게(5:3:2:1).
BOARD_WEIGHTS = {
    "스카이차":       5,   # 최상위 단일 키워드
    "스카이차 임대":   3,   # 핵심 상업 클러스터 (실검색어 표준 — 舊 일대 대체)
    "스카이차 비용":   3,
    "스카이차 가격":   3,
    "근처 스카이차":   3,   # 舊 요금 대체
    "스카이차 업체":   3,   # 舊 이용료 대체
...
# [휴면] 사이트별 지역 분담 (미러/카니발 회피) — 가치 기반 분할.
# 2026-07-16 사장님 지시로 두 사이트(ajasky·wormsz1) 모두 전국 발행으로 통일 —
# 두 워크플로(publish.yml·publish-wormsz1.yml)에서 SITE_SHARD env 를 제거해 비활성.
# 코드는 하위호환·재활성 대비로 유지 (SITE_SHARD 미설정 → 전 지역 = 현재 동작).
# (구 분담: shard 0 = 수도권+충청+지방광역시, shard 1 = 전북·전남·경북·경남)
_SHARD0_PARENTS = {
...
def pick_target():
    ...
    1) 레벨 우선순위: 시·군·구를 먼저 채운다. 법정동은 검색량이 거의 0이라
       (메모리 실측) 후순위 — 모든 시군구가 DONG_GATE 건 이상 쌓이기 전엔
       후보에서 제외한다. 결과적으로 "시군구 전국 1바퀴 → 시군구 심화 → 법정동" 순서.
       (광역 단독 발행은 2026-07-16부터 제외 — regions.all_region_targets 참조.)
    ...
    # 기본 6 ≈ 시군구당 핵심 보드 한 바퀴(스카이차+임대/업체/근처/비용/가격).
