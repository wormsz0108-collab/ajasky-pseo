...
#   렌탈/대여      : 사업 절대 금지어 ("임대"만 사용 — 2026-07-16 보드 교체와 함께 확정)
#   일대           : 舊 보드 키워드 — 실검색어 아님, 신규 유입 차단 (기존 글은 불변)
#   무료/0원/추가요금없음/최저가/도착시간 : 지키지 못할 절대약속 표현 금지
#   주간과 동일     : 야간·주말 금액 다름 명시 규칙 위반
#   윤재근         : 대표 본명 노출 금지
#   날씨           : 기상 화제 금지 (무관 검색 유입 방지)
#   3.5톤/굴절     : 미보유 장비 표기 금지 (1톤·3톤·5톤만 — 사장님 2026-07-14 확정)
#   출장비/추가비용/숨겨진비용 없음 : 정적 사이트 게이트(_audit_forbidden.py)와 동일 수준으로
#                                    맞춤 — "추가 요금"만 막으면 "출장비 없이"로 새어 나감
#   N분 이내 출동/회신 : 도착뿐 아니라 출동·회신도 시간 확약이라 동일 취급
#   야간/주말/철야/공휴일 + 동일 : "주간과 동일" 외 표현도 차단
BANNED_RE = re.compile(
    r"렌탈|대여|일대|무료|무상|0\s*원|추가\s*요금\s*없|추가요금\s*없|요금\s*없이|비용\s*없이"
    r"|추가\s*비용[이은는]?\s*없|숨겨진\s*(?:추가\s*)?(?:비용|요금)[이은는]?\s*없"
    r"|출장비\s*없[이는음]"
    r"|최저가|\d+\s*분\s*(?:이내\s*|내\s*|만에\s*|안에\s*)?(?:도착|출동|회신)|즉시\s*도착"
    r"|(?:주간|야간|주말|철야|공휴일)[^.]{0,25}동일(?:한|하게)?\s*(?:금액|요금|가격|비용|단가)"
    r"|동일(?:한)?\s*(?:금액|요금|가격|비용|단가)[^.]{0,25}(?:야간|주말|철야|공휴일)"
    r"|윤재근|날씨"
    r"|3\.5\s*톤|굴절"
)

# 장비 톤수 화이트리스트 — 3.5톤·굴절만 막으면 "2.7톤","10톤","17.5톤" 같은 허위 표기가
# 그대로 통과한다. 보유 장비는 1톤·3톤·5톤뿐(사장님 2026-07-14 확정).
TON_RE = re.compile(r"(\d+(?:\.\d+)?)\s*톤")
TON_ALLOWED = {"1", "3", "5"}
...
def _banned_hits(d: dict[str, Any]) -> list[str]:
    ...
    for t in texts:
        ...
        # 허용 톤수(1·3·5) 외 숫자+톤 표기는 미보유 장비 홍보 → 위반
        hits.extend(f"{v}톤" for v in TON_RE.findall(t or "") if v not in TON_ALLOWED)
    return hits
